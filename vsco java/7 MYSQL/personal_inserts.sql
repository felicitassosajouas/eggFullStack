...
-- muestro los nombres de quienes son secretarias y vendedores:
select nombre from empleados where cargo_emp = 'secretaria';
-- en orden alfabético y viceversa:
select nombre from empleados where cargo_emp = 'vendedor' order by nombre asc;
select nombre from empleados where cargo_emp = 'vendedor' order by nombre desc;
...
-- concatenar información con el nombre de 'NOMBRE Y CARGO'
select CONCAT(nombre, '. CARGO -->', cargo_emp) As 'NOMBRE Y CARGO' From empleados;
...
/*Obtener total a pagar a empleados del departamento 3000: salario + comisión + 500. 
Mostrar nombre y el total a pagar alfabéticamente*/
...
select nombre, sal_emp, comision_emp, sal_emp + comision_emp + 500 as 'total a pagar' from empleados where comision_emp > 1000;
select nombre, sal_emp, comision_emp, sal_emp + comision_emp + 500 as 'total a pagar' from empleados where comision_emp = 0;
select nombre from empleados where comision_emp > sal_emp;
select nombre from empleados where sal_emp * 0.3 >= comision_emp;
-- Hallar los empleados cuyo nombre no contiene la cadena “MA”
select nombre from empleados where nombre not like '%ma%';
-- Obtener los nombres de departamentos que sean “Ventas”, “Investigación” o ‘Mantenimiento'.
-- IN
select * from departamentos where nombre_depto in ('mantenimiento' , 'ventas', 'investigacion');
-- NOT IN 
select * from departamentos where nombre_depto not in ('mantenimiento' , 'ventas', 'investigacion');
-- MAXIMO SALARIO
select max(sal_emp) from empleados;
select max(nombre) as 'último alfabéticamente' from empleados order by nombre asc;
select max(sal_emp) as 'salario maxima', min(sal_emp) as 'salario minimo', (max(sal_emp) - min(sal_emp)) as 'diferencia' from empleados as diferencia;
